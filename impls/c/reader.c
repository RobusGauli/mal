...
Node node_vector__new(cvector_nodes_t *cvector_nodes) {
  Node node = {.nodetype = NODE__VECTOR,
               .nodeval = {.nodevector = {.mem = cvector_nodes}}};
  return node;
}
...
Node node_symbol__new(char *mem, size_t len) {
  return (Node){
      ...
      .nodeval = {.nodesymbol = {.stringview = {.mem = mem, .len = len}}}};
}

Node node_comment__new(char *mem, size_t len) {
  return (Node){
      .nodetype = NODE__COMMENT,
      .nodeval = {.nodecomment = {.stringview = {.mem = mem, .len = len}}}};
}
